/**
 * 縦横の文字数とCanvasのImageDataからAAを生成するクラス
 */
export class AARenderer {
    charTable;
    rParam;
    gParam;
    bParam;
     ...
     */
    constructor() {
        this.charTable = new ArrayBuffer(256 * 256);
        this.rParam = 0.298912;
        this.gParam = 0.586611;
        this.bParam = 0.114478;
    ...
    createCharTable() {
        ...
        for (let i = 0; i < charNum; i++) {
            const avg = (brightness[i][0] + brightness[i][1]) / 2;
        ...
        for (let i = 0; i < 256; i++) {
            for (let j = 0; j < 256; j++) {
                ...
                this.charTable[i * 256 + j] = chars[indexMin];
            }
        }
    };
    ...
    render(img, cfw, cfh) {
        ...
        for (var y = 0; y < cfh; y++) {
            ...
            for (var x = 0; x < cfw; x++) {
                ...
                res += this.charTable[
                    ((imgData[idx1] * this.rParam +
                        imgData[idx1 + 1] * this.gParam +
                        imgData[idx1 + 2] * this.bParam | 0) << 8) +
                    ((imgData[idx2] * this.rParam +
                        imgData[idx2 + 1] * this.gParam +
                        imgData[idx2 + 2] * this.bParam | 0))
                ];
            }
